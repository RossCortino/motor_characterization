function [] = sigasterisk(xbar, ybar, xgroup, ygroup, str_show, bars, varargin)
    ...
    %Text
    text(mean([P2_x, P3_x])+threshold(4), mean([P2_y, P3_y])+threshold(3) * sign(mean([P2_y, P3_y])) - threshold(3) * (sign(mean([P2_y, P3_y])) == -1), str_show, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle','FontSize',10);
