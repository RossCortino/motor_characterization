...
def readData(on_target):
    ...
    if curr_motor == 1: # MN1005 in current control
        csv_writer.writerow([t, curr_motor, MN1005.current_command_mA, Split.vel_command_RPM, on_target,\
                             vel_MN1005, vel_split, i_MN1005, i_split, futek_torque])
    elif curr_motor == 2: # Split Motor in current control
        csv_writer.writerow([t, curr_motor, Split.current_command_mA, MN1005.vel_command_RPM, on_target,\
                             vel_MN1005, vel_split, i_MN1005, i_split, futek_torque])

    return vel_MN1005, vel_split, i_MN1005, i_split, futek_torque
...
def main():
    ...
    global t0, res_torque, curr_motor, MN1005, Split, csv_writer
    ...
    try:
        ...
        # curr_motor = 1 # 1: MN1005 current control, split motor velocity control
        # with open("data/efficiency_PAIRED_MN1005-curr_Split-vel_%s.csv"% csv_timestamp,'w') as fd:
        #     csv_writer = csv.writer(fd)
        #     csv_writer.writerow(csv_header)
        #     hitVelTargets(vel_targets, i_targets, Vel_Motor=Split, Curr_Motor=MN1005)
        curr_motor = 2 # 2: split motor current control, MN1005 velocity control
        with open("data/efficiency_PAIRED_Split-curr_MN1005-vel_%s.csv"% csv_timestamp,'w') as fd:
            csv_writer = csv.writer(fd)
            csv_writer.writerow(csv_header)
            hitVelTargets(vel_targets, i_targets, Vel_Motor=MN1005, Curr_Motor=Split)
...
if __name__ == '__main__':
    ...
    csv_header = ["t", "curr_motor", "curr_target_mA", "vel_target_RPM", "on_target",\
                  "vel_MN1005_RPM", "vel_split_RPM", "i_q_MN1005_mA", "i_q_split_mA", "futek_torque_Nm"]
    # with open("data/efficiency_MN1005-vel_Split-curr_%s.csv"% datetime.now().strftime("%Y-%b-%d-%H%M%S"),'w') as fd:
    #     csv_writer = csv.writer(fd)
